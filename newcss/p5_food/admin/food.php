...
<body>
    ...
    <a class="a-out" href="food_add.php">เพิ่มเมนูอาหาร</a>
<br>
<br>
<div class="d-fixed jcc">
    ...
    <?php foreach($log as $row){ 
        ...
        <?php echo $row['status'] == '1'?"<a class='showstatusfood-warning' href='?status1={$row['food_id']}'>วัตถุดิบไม่พร้อม</a>" : "<a class='showstatusfood-havestock' href='?status2={$row['food_id']}'>พร้อมขาย</a>";  ?>
        <br>
        <div class="btn-between">
        <a href="food_edit.php?id=<?php echo $row['food_id'] ?>">เเก้ไขอาหาร</a>
        <a href="?del=<?php echo $row['food_id'] ?>">ลบ</a>
        </div>
        </div>
    </div>
        <?php } ?>
